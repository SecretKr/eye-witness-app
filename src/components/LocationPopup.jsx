    ...
    >
      ...
      >
        ...
        <div className="relative h-full flex flex-col">
          ...
          <div
            className={`relative w-full transition-all duration-500 overflow-hidden ${expanded ? "h-48 opacity-100" : "h-0 opacity-0"}`}
          >
            <img
              src={image}
              alt={name}
          ...
          {/* Content Container */}
          <div
            className={`p-5 flex flex-col items-center justify-center ${expanded ? "flex-1 pt-2" : "h-full"}`}
          >
            {/* Header */}
            <div className="text-center w-full">
              ...
              <div className="flex items-center justify-center gap-1 mb-2">
                <span className="text-[10px] font-bold text-white/90 uppercase tracking-wider mr-1">
                  Safety Rating
                </span>
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      size={12}
                      className={`${star <= Math.round(rating) ? "fill-white text-white" : "text-white/30"}`}
                    />
                  ))}
                </div>
